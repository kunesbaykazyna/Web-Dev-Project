...
@Component({
    ...
    <div class="profile-page">
      <div class="profile-layout">
        ...
        <main class="profile-content">
          ...
          <section class="content-card">
            ...
            <div class="form-actions">
              <button
                class="primary-btn"
                (click)="submitReview()"
                [disabled]="!newReview.text || !newReview.book_title"
              >
                Post Review
              </button>
            </div>
          ...
          <section class="content-card" *ngIf="myReviews().length > 0">
            <div class="section-head">
              <div>
                <h3>My Reviews</h3>
            ...
            <div class="reviews-list">
              <div class="review-card" *ngFor="let r of myReviews()">
                <div class="review-card-head">
                  ...
                  <div class="review-date">{{ r.created_at | date:'dd.MM.yyyy' }}</div>
                </div>
  ...
  styles: [`
    ...
    .profile-page {
      ...
      radial-gradient(circle at bottom right, rgba(2, 195, 154, 0.08), transparent 30%),
      #f8fafb;
    }
    ...
    .profile-cover {
      height: 110px;
      background: linear-gradient(135deg, #115754, rgba(70, 174, 169, 0.89));
    }
    ...
    .profile-avatar {
      ...
      background: linear-gradient(135deg, #0b615a, rgb(44, 184, 179));
      color: #ffffff;
      display: flex;
      align-items: center;
      justify-content: center;
      font-size: 34px;
      font-weight: 700;
      box-shadow: 0 8px 18px rgba(0, 168, 150, 0.25);
    }
...
export class ProfileComponent implements OnInit {
  ...
  loadMyReviews() {
    ...
    this.http.get<any[]>('http://127.0.0.1:8000/api/reviews/favorites/', { headers }).subscribe({
  ...
  submitReview() {
    ...
    this.http.get<any[]>(`http://127.0.0.1:8000/api/books/?search=${encodeURIComponent(title)}`, { headers }).subscribe({
      next: (books: any[]) => {
        const foundBook =
            books.find(b => b.title?.toLowerCase() === title.toLowerCase()) ||
            books.find(b => b.title?.toLowerCase().includes(title.toLowerCase()));

        if (!foundBook) {
          this.reviewError = 'Book not found.';
          this.reviewSuccess = false;
          return;
        }

        this.http.post(
            `http://127.0.0.1:8000/api/reviews/books/${foundBook.id}/reviews/`,
          ...
          next: (res: any) => {
            ...
            this.reviewError = '';
            this.myReviews.set([res, ...this.myReviews()]);
            this.newReview = {
              book_title: '',
              text: '',
              rating: 5
          ...
          error: (err) => {
            this.reviewError = err.error?.detail || 'Failed to post review.';
            this.reviewSuccess = false;
          }
        });
      },
      error: () => {
        this.reviewError = 'Could not find the book by title.';
        this.reviewSuccess = false;
      }
    });
  }
